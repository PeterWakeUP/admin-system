package com.evsoft.modules.sys.oauth2;
...
public class OAuth2Realm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        //用户权限列表
        Set<String> permsSet = TokenDBMap.permsSetDBMap.get(userId);
        if(null==permsSet){
            permsSet = shiroService.getUserPermissions(userId);
            TokenDBMap.permsSetDBMap.put(userId,permsSet);
        }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        if(tokenEntity == null || tokenEntity.getExpireTime().getTime() < System.currentTimeMillis()){
            TokenDBMap.tokenDBMap.remove(accessToken);
